...
from src.feature_discovery.config import NEO4J_HOST, NEO4J_CREDENTIALS, NEO4J_DATABASE
from src.feature_discovery.graph_processing.neo4j_queries import (
...
def ensure_database_exists(db_name):
    with driver.session(database="system") as session:
        result = session.run("SHOW DATABASES YIELD name WHERE name = $name RETURN count(*) > 0 AS exists", name=db_name)
...
ensure_database_exists(NEO4J_DATABASE)
