...
from src.feature_discovery.config import DATA_FOLDER, RESULTS_FOLDER, ROOT_FOLDER
from src.feature_discovery.experiments.ablation import autofeat
from src.feature_discovery.experiments.baselines import non_augmented, arda, join_all, join_all_bfs
from src.feature_discovery.experiments.dataset_object import Dataset
from src.feature_discovery.experiments.evaluate_join_paths import evaluate_paths_from_file
...
from src.feature_discovery.graph_processing.neo4j_transactions import export_dataset_connections, export_all_connections
...
def get_tfd_results(dataset: Dataset, algorithm: str, top_k: int = 15, value_ratio: float = 0.65, fit_model=False) -> List:
    spearman_mrmr_results, top_k_paths = autofeat(
        dataset=dataset, top_k=top_k, value_ratio=value_ratio, algorithm=algorithm, fit_model=fit_model
    )
