...
from src.feature_discovery.autofeat_pipeline.autofeat import AutoFeat
from src.feature_discovery.autofeat_pipeline.join_path_utils import get_path_length
from src.feature_discovery.config import RESULTS_FOLDER
from src.feature_discovery.experiments.evaluation_algorithms import evaluate_all_algorithms
from src.feature_discovery.experiments.init_datasets import ALL_DATASETS
from src.feature_discovery.experiments.result_object import Result
...
def evaluate_paths(bfs_result: AutoFeat, problem_type: str, algorithm: str, top_k_paths: int = 15, fit_model:bool =False) -> Tuple[List[Result], List[Tuple]]:
    ...
    print("top k path list ", top_k_path_list)
    ...
    for path in tqdm.tqdm(top_k_path_list):
        ...
        print("features to be evaluated ",features)
        ...
        print("result data frame from evaluation ", dataframe)

        if len(features) < 2:
            features = bfs_result.partial_join_selected_features[bfs_result.base_table_id]
            features.append(bfs_result.target_column)

        # without fitting the ml models
        if fit_model:
            ...
            result = Result(
                algorithm=None,
                accuracy=None,
                join_path_features=list(features),
                data_path=path_list
            )
