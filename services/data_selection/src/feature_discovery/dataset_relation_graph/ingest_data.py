...
from src.feature_discovery.config import CONNECTIONS, DATA_FOLDER, SLASH
from src.feature_discovery.dataset_relation_graph.dataset_discovery import profile_valentine_all, profile_valentine_dataset
from src.feature_discovery.experiments.dataset_object import Dataset
from src.feature_discovery.graph_processing.neo4j_transactions import merge_nodes_relation_tables, create_node
...
def ingest_unprocessed_data(dataset_folder_name: str = None):
    ...
    print( "current working fils ", files)
    for f in files:
        table_path = f.partition(f"{DATA_FOLDER}/")[2]
        table_name = os.path.basename(Path(f))
