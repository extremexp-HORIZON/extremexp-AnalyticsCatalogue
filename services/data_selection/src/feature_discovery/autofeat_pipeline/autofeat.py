...
from src.feature_discovery.autofeat_pipeline.join_data import join_and_save
from src.feature_discovery.autofeat_pipeline.join_path_feature_selection import RelevanceRedundancy
from src.feature_discovery.autofeat_pipeline.join_path_utils import compute_join_name
from src.feature_discovery.experiments.dataset_object import CLASSIFICATION
from src.feature_discovery.graph_processing.neo4j_transactions import (
    get_adjacent_nodes,
    get_relation_properties_node_name,
)
from src.feature_discovery.helpers.read_data import get_df_with_prefix
...
class AutoFeat:
    ...
    def streaming_feature_selection(self, queue: set, previous_queue: set = None):
        ...
        while len(queue) > 0:
            ...
            print("current node ", base_node_id)
            print("neighbors of current node", neighbours)
            print("previous queue ", previous_queue)
            ...
            for node in neighbours:
                ...
                while len(previous_queue) > 0:
                    ...
                    for prop in highest_ranked_join_keys:
                        ...
                        # print("previous dataframe ", previous_join.head())
                    
                        # Step - Join
                        print("left dataframe ", previous_join.head())
                        print("right dataframe columns", right_df.columns)
                        print("left dataframe columns ", previous_join.columns)
                        ...
                        print("join filename ", join_filename)

                        # print("after dataframe ", joined_df.head())
                        ...
                        print("join key properties for data quality check ", prop)
                        ...
                        print("relevancy results ", result)
    ...
    def step_data_quality(self, join_key_properties: tuple, joined_df: pd.DataFrame) -> bool:
        ...
        # use left_table because join removes the right join key, akin to sql 
        try:
            if joined_df[f"{from_table}.{join_prop['to_column']}"].count() / joined_df.shape[0] < self.value_ratio:
