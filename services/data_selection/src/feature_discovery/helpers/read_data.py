...
from src.feature_discovery.config import DATA_FOLDER
from src.feature_discovery.graph_processing.neo4j_transactions import get_node_by_id
...
def get_df_with_prefix(
    ...
    if use_polars:
        ...
        if target_column:
            ...
            dataframe = dataframe.select(pl.all().name.map(lambda col_name: f"{node_label}.{col_name}"))
