from typing import Optional, List
from fastapi import FastAPI, File, UploadFile, Form
from fastapi.responses import FileResponse, StreamingResponse
import shutil
from pathlib import Path
import os
import aiofiles
from src.feature_discovery.experiments.init_datasets import init_datasets

import numpy as np

np.random.seed(42)

import pandas as pd
import tqdm
import typer
from typing_extensions import Annotated

# from feature_discovery.config import RESULTS_FOLDER, DATA_FOLDER
# from feature_discovery.dataset_relation_graph.dataset_discovery import profile_valentine_dataset, profile_valentine_all
# from feature_discovery.dataset_relation_graph.ingest_data import ingest_nodes, ingest_data_with_pk_fk
# from feature_discovery.experiments.init_datasets import ALL_DATASETS
# from feature_discovery.experiments.utils_dataset import filter_datasets
# from feature_discovery.run import (
#     get_arda_results,
#     get_base_results,
#     get_tfd_results,
#     get_all_results,
#     get_results_tune_value_ratio_classification,
#     get_results_tune_k,
#     get_join_all_results,
#     get_autofeat_ablation,
# )

from src.feature_discovery.config import RESULTS_FOLDER, DATA_FOLDER
from src.feature_discovery.dataset_relation_graph.dataset_discovery import profile_valentine_dataset, profile_valentine_all
from src.feature_discovery.dataset_relation_graph.ingest_data import ingest_nodes, ingest_data_with_pk_fk
from src.feature_discovery.experiments.init_datasets import ALL_DATASETS
from src.feature_discovery.experiments.dataset_object import Dataset
from src.feature_discovery.experiments.utils_dataset import filter_datasets
from src.feature_discovery.run import (
    get_arda_results,
    get_base_results,
    get_tfd_results,
    get_all_results,
    get_results_tune_value_ratio_classification,
    get_results_tune_k,
    get_join_all_results,
    get_autofeat_ablation,
)

app = FastAPI()

@app.get("/datasets")
def datasets():
    file_location = DATA_FOLDER
    existing_dir = list(os.listdir(file_location))
    return {"datasets list": existing_dir}

@app.post("/select_data")
async def select_data(    
        data_name : str = Form("default"),
        thresholding: float = Form(0.55),
        discovery_method : str = Form("COMA"),
        discovery_strategy: str = Form("instance"),
        target_label:str = Form("label"),
        base_table: str = Form(...),
        job_type: str=Form("regression"),
        files: List[UploadFile] = File(...),
        save_relations = Form(False)):
    """
    Files be the files from which to select data
    Params be used for deciding what functions to use.
    """
    file_location = DATA_FOLDER
    base_path = DATA_FOLDER / base_table

    existing_dir = list(os.listdir(file_location))
    # print("before save ", existing_dir)

    if data_name not in existing_dir:
        new_dir = Path(DATA_FOLDER / data_name)
        new_dir.mkdir()
        new_res_dir = Path(RESULTS_FOLDER / data_name)
        new_res_dir.mkdir()
    else:
        new_dir = Path(DATA_FOLDER / data_name)
        new_res_dir = Path(RESULTS_FOLDER / data_name)

    dataset_path = DATA_FOLDER / "datasets.csv"
    datasets = pd.read_csv(dataset_path)
    found = False
    for dataset in datasets["base_table_path"]:
        if dataset == data_name :       
            found = True
            break
    else:
        with open(dataset_path, "a") as temp:
            # base_table_path, base_table_name, base_table_label, target_column,dataset_type
            temp.write(f"\n{data_name},{base_table},{data_name},{target_label},{job_type}")

    new_dataset = Dataset(  base_table_label = data_name, 
                            base_table_path = Path(data_name),
                            base_table_name = Path(base_table),
                            target_column= target_label,
                            dataset_type = job_type)
    
    # print("Uploaded files from api endpoint ", files)

    files_in_directory = list(os.listdir(new_dir))
    for f in files:
        # Read or stream the content (here we read fully for the demo)
        dest = new_dir / f.filename
        with open(dest, 'wb') as out_file:
            while True:
                content = await f.read(1024)  # async read chunk
                if content:
                    out_file.write(content)  # async write chunk
                else:
                    break

    init_datasets()
    from src.feature_discovery.experiments.init_datasets import ALL_DATASETS
    ALL_DATASETS.add(new_dataset)
    existing_dir = list(os.listdir(file_location))
    files_in_new_dir = os.listdir(new_dir)

    # print("files in directory ", files_in_new_dir)
    has_connections = None
    for file in files_in_new_dir:
        if "connections" in file:
            has_connections = True
            break
    else:
        has_connections = False
    
    # print("has connections ", has_connections)

    if has_connections:
        ingest_data_with_pk_fk(new_dataset)
    else:
        ingest_data(data_name, thresholding, True, discovery_method, discovery_strategy)
    
    run_autofeat(dataset = new_dataset)
    response_file = RESULTS_FOLDER / f"{data_name}_AutoFeat.csv"
    res_df = pd.read_csv(response_file)
    responseObject = FileResponse(response_file, media_type="text/csv", filename = "{data_name}_AutoFeat.csv")

    # print("after_save ", existing_dir)
    return responseObject


@app.post("/run_arda")
def run_arda(
    dataset_labels: Annotated[
        Optional[List[str]], typer.Option(help="Whether to run only on a list of datasets. Filters by dataset labels")
    ] = None,
    algorithm: Annotated[str, typer.Option(help="ML algorithm for evaluation: [RF, GBM, XT, XGB, KNN, LR1]")] = None,
    results_file: Annotated[str, typer.Option(help="CSV file where the results will be written")] = "results_arda.csv",
):
    """Runs the ARDA experiments."""
    all_results = []
    datasets = filter_datasets(dataset_labels)
    for dataset in tqdm.tqdm(datasets):
        all_results.extend(get_arda_results(dataset, algorithm))

    pd.DataFrame(all_results).to_csv(RESULTS_FOLDER / results_file, index=False)


@app.post("/base")
def run_base(
    dataset_labels: Annotated[
        Optional[List[str]], typer.Option(help="Whether to run only on a list of datasets. Filters by dataset labels")
    ] = None,
    algorithm: Annotated[str, typer.Option(help="ML algorithm for evaluation: [RF, GBM, XT, XGB, KNN, LR1]")] = None,
    results_file: Annotated[str, typer.Option(help="CSV file where the results will be written")] = "results_base.csv",
):
    """Runs the base experiments."""
    all_results = []
    datasets = filter_datasets(dataset_labels)
    for dataset in tqdm.tqdm(datasets):
        all_results.extend(get_base_results(dataset, algorithm))

    pd.DataFrame(all_results).to_csv(RESULTS_FOLDER / results_file, index=False)


@app.post("/run_join_all")
def run_join_all(
    dataset_labels: Annotated[
        Optional[List[str]], typer.Option(help="Whether to run only on a list of datasets. Filters by dataset labels")
    ] = None,
    algorithm: Annotated[str, typer.Option(help="ML algorithm for evaluation: [RF, GBM, XT, XGB, KNN, LR1]")] = None,
    results_file: Annotated[
        str, typer.Option(help="CSV file where the results will be written")
    ] = "results_join_all.csv",
):
    """Runs 4 experiments: join all tables in BFS and DFS order, then same approaches with filter
    feature selection: spearman, and with wrapper feature selection: forward selection"""

    all_results = []
    datasets = filter_datasets(dataset_labels)
    for dataset in tqdm.tqdm(datasets):
        all_results.extend(get_join_all_results(dataset, algorithm))

    pd.DataFrame(all_results).to_csv(RESULTS_FOLDER / results_file, index=False)


@app.post("/run_abalation")
def run_ablation(
    top_k: Annotated[int, typer.Option(help="Number of features and paths")] = 15,
    dataset_labels: Annotated[
        Optional[List[str]], typer.Option(help="Whether to run only on a list of datasets. Filters by dataset labels")
    ] = None,
    algorithm: Annotated[str, typer.Option(help="ML algorithm for evaluation: [RF, GBM, XT, XGB, KNN, LR1]")] = None,
    results_file: Annotated[
        str, typer.Option(help="CSV file where the results will be written")
    ] = "results_tfd_ablation.csv",
    value_ratio: Annotated[float, typer.Option(help="Value ratio to be used in the TFD experiments")] = 0.65,
):
    """Runs the 5 types of TFD experiments: Spearman + JMI, Pearson + MRMR, Pearson + JMI,
    (no relevance) MRMR, (no redundancy) Spearman
    """
    all_results = []
    datasets = filter_datasets(dataset_labels)
    for dataset in tqdm.tqdm(datasets):
        all_results.extend(get_autofeat_ablation(dataset, algorithm, top_k, value_ratio))

    pd.DataFrame(all_results).to_csv(RESULTS_FOLDER / results_file, index=False)


# @app.command("/run_autofeat")
def run_autofeat(
    top_k: Annotated[int, typer.Option(help="Number of features and paths")] = 15,
    dataset: Dataset = None,
    dataset_labels: Annotated[
        Optional[List[str]], typer.Option(help="Whether to run only on a list of datasets. Filters by dataset labels")
    ] = None,
    fit_model: Annotated[
        Optional[bool], typer.Option(help="Whether to fit any ml models or not, true = yes, false = no.")] = False ,
    algorithm: Annotated[str, typer.Option(help="ML algorithm for evaluation: [RF, GBM, XT, XGB, KNN, LR]")] = None,
    results_file: Annotated[
        str, typer.Option(help="CSV file where the results will be written")
    ] = "results_autofeat.csv",
    value_ratio: Annotated[float, typer.Option(help="Value ratio to be used in the TFD experiments")] = 0.65,
):
    """Runs the AutoFeat experiments."""
    
    if dataset is None and len(dataset_labels) == 0:
        # run for all datasets in dtaabase if no dataset named
        all_results = []
        datasets = filter_datasets(dataset_labels)
        for dataset in tqdm.tqdm(datasets):
            all_results.extend(get_tfd_results(dataset, algorithm, top_k, value_ratio))
    else:
        results_file = f"{dataset.base_table_label}_autofeat.csv"
        all_results = get_tfd_results(dataset, algorithm, top_k, value_ratio)

    result_path = RESULTS_FOLDER / results_file

    pd.DataFrame(all_results).to_csv(result_path, index=False)
    


# @app.command("/run_all")
def run_all(
    dataset_labels: Annotated[
        Optional[List[str]],
        typer.Option(help="Whether to run only on a list of datasets. Filters by dataset labels"),
    ] = None,
    algorithm: Annotated[str, typer.Option(help="ML algorithm for evaluation: [RF, GBM, XT, XGB, KNN, LR]")] = None,
    results_file: Annotated[
        str, typer.Option(help="CSV file where the results will be written")
    ] = "all_results_autogluon.csv",
    join_all: Annotated[
        bool, typer.Option(help="Run the Join-All experiments")
    ] = True,
):
    """Runs all experiments (ARDA + Base + TFD + Join-All)."""
    get_all_results(dataset_labels, algorithm, results_file, join_all)


# @app.command("/run_tune_value_ratio")
def run_tune_value_ratio(
    dataset_labels: Annotated[
        Optional[List[str]],
        typer.Option(help="Whether to run only on a list of datasets. Filters by dataset labels"),
    ] = None,
    results_file: Annotated[
        str, typer.Option(help="CSV file where the results will be written")
    ] = "all_results_value_ratio_tuning.csv",
):
    """Run experiment on the sensitivity of value_ratio hyper-parameter"""
    datasets = filter_datasets(dataset_labels)
    get_results_tune_value_ratio_classification(datasets, results_file)


# @app.command("/run_tune_top_k")
def run_tune_top_k(
    dataset_labels: Annotated[
        Optional[List[str]],
        typer.Option(help="Whether to run only on a list of datasets. Filters by dataset labels"),
    ] = None,
    results_file: Annotated[
        str, typer.Option(help="CSV file where the results will be written")
    ] = "all_results_top_k_features_tuning.csv",
):
    """Run experiment on the sensitivity of value_ratio hyper-parameter"""
    datasets = filter_datasets(dataset_labels)
    get_results_tune_k(datasets, results_file)


# @app.command("/ingest_kfk_data")
def ingest_kfk_data(
    dataset_label: Annotated[
        Optional[str],
        typer.Option(help="The label of the dataset to ingest"),
    ] = None,
    discover_connections_dataset: Annotated[
        bool, typer.Option(help="Run dataset discovery to find more connections within the dataset")
    ] = False,
):
    """Ingest the new dataset into neo4j database"""
    if not dataset_label:
        datasets = ALL_DATASETS
    else:
        datasets = filter_datasets([dataset_label])

    if len(datasets) == 0:
        raise typer.BadParameter(
            "Incorrect dataset label. The label should have the same value with <base_table_label>."
        )

    for dataset in tqdm.tqdm(datasets):
        ingest_data_with_pk_fk(dataset=dataset, profile_valentine=discover_connections_dataset)


# @app.command("/ingest_data")
def ingest_data(
        dataset: str , 
    data_discovery_threshold: Annotated[
        float,
        typer.Option(
            help="Run dataset discovery to find more connections within the entire data lake with given"
            " accuracy rate threshold"
        ),
    ] = None,
    discover_connections_data_lake: Annotated[
        bool, typer.Option(help="Run dataset discovery to find more connections within the entire data lake")
    ] = False,
    method: Annotated[
        str, 
        typer.Option(help="Define the stratgey used for valentine to do the relation discovery")]="COMA",
    strategy:Annotated[
        str, 
        typer.Option(help="The subs-strategy of the method that will be used by the matcher")]="instance"
):
    """
    Ingest all dataset from specified "data" folder.
    """
    files = ingest_nodes(dataset)

    if data_discovery_threshold and discover_connections_data_lake:
        profile_valentine_all(dataset, valentine_threshold=data_discovery_threshold, files=files, method=method, strategy=strategy)
        return

    if data_discovery_threshold and not discover_connections_data_lake:
        for dataset in ALL_DATASETS:
            profile_valentine_dataset(dataset.base_table_label, valentine_threshold=data_discovery_threshold)


# if __name__ == "__main__":
#     app()
